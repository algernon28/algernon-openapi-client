...
public class WorkflowAssociationStatusMapping {
  ...
  static {
    ...
    openapiFields.add("oldStatusId");

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>();
    openapiRequiredFields.add("newStatusId");
    openapiRequiredFields.add("oldStatusId");
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!WorkflowAssociationStatusMapping.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in WorkflowAssociationStatusMapping is not found in the empty JSON string", WorkflowAssociationStatusMapping.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!WorkflowAssociationStatusMapping.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `WorkflowAssociationStatusMapping` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (!jsonObj.get("oldStatusId").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format("Expected the field `oldStatusId` to be a primitive type in the JSON string but got `%s`", jsonObj.get("oldStatusId").toString()));
      }
  }
